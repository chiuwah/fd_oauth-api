...
func TestAccessTokenConstants(t *testing.T) {
	assert.EqualValues(t, 24, expirationTime, "brand new access token should not be expired")
}

func TestGetNewAccessToken(t *testing.T) {
	at := GetNewAccessToken()
	assert.False(t, at.IsExpired(), expirationTime, "brand new access token should not be expired")
